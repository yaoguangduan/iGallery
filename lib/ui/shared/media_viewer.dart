...
class _MediaViewerState extends State<MediaViewer> with TickerProviderStateMixin {
  ...
  Future<void> _downloadCurrent() async {
    final item = _item;
    try {
      final savePath = await DownloadService.instance.pickSavePath(item.filename);
      if (savePath == null || !mounted) return; // 用户取消
      await DownloadService.instance.downloadTo(widget.service, item.id, savePath);
      final name = savePath.split(Platform.pathSeparator).last;
      if (mounted) showToast(context, '已下载 $name', kind: ToastKind.success);
    } on ApiException catch (e) {
